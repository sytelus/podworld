...
class Env(object):
    ...
    def handle_collisions(self):
        ...
        def begin_wall_collision(arbiter, space, *args, **kwargs):
            ...
            if self.agent.body.position[0]<=25:
                self.agent.body.position = (58,self.agent.body.position[1])

            if self.agent.body.position[0]>=59:
                self.agent.body.position = (26,self.agent.body.position[1])

            if self.agent.body.position[1]<=25:
                self.agent.body.position = (self.agent.body.position[0],58)

            if self.agent.body.position[1]>=59:
                self.agent.body.position = (self.agent.body.position[0],26)
